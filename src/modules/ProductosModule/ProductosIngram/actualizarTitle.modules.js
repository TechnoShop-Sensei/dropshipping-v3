...
class AbasteoScraper {
  ...
  async fetchPartNumbers() {
    ...
    const [rows] = await this.pool.execute('SELECT Modelo, Sku_ingram, id_producto FROM ingramProductosv2 WHERE id_woocommerce_producto IS NULL AND id_producto >= 7128');
    return rows;
  }
